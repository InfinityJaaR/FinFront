...
import SuavizamientoExponencialService from '@/services/GestionEmpresas/Ventas/SuavizamientoExponencialService'
import { useModal } from '@/context/ModalContext'
import { Card, CardHeader, CardTitle, CardContent } from '@/components/ui/card'
import Button from '@/components/ui/Button'
import { Tabs, TabsList, TabsTrigger, TabsContent } from '@/components/ui/tabs'
import { Pencil, Trash2, Eye, Download, FileSpreadsheet } from 'lucide-react'
          ...
          <Tabs value={tab} onValueChange={setTab} className="w-full">
            ...
            <TabsContent value="meses" className="mt-4 space-y-4">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <div className="flex flex-col bg-green-700 border border-green-200 text-green-700 px-3 py-1 rounded rounded-2xl text-xs cursor-pointer">
                  <div className="flex flex-row items-center gap-1">
                    <FileSpreadsheet className="w-4 h-4 text-green-200 mb-1" />
                    <label htmlFor="excel_upload" className="text-xs font-medium text-white mb-1 cursor-pointer">Cargar desde Excel</label>
                  </div>
                  <input
                    id="excel_upload"
                    type="file"
                    ...
                    className="text-xs font-bold text-green-200 cursor-pointer"
                            ...
                            <div className="flex justify-end items-center gap-2">
                              ...
                              {mes === 12 && (
                                <Button
                                  onClick={async () => {
                                    try {
                                      // Validar meses 1..11 completos
                                      const validCount = rows.filter(x => x.mes < 12 && x.monto !== '' && x.monto != null && !Number.isNaN(Number(x.monto))).length
                                      if (validCount < 11) {
                                        await alert({ title: 'Datos incompletos', message: `Se requieren al menos 11 meses (1..11) para generar M12. Encontrados: ${validCount}.` })
                                        return
                                      }
                                      // Si M12 ya tiene valor, confirmar sobrescritura
                                      if (r.monto !== '' && r.monto != null) {
                                        const ok = await confirm({ title: 'Sobrescribir Mes 12', message: 'Mes 12 ya tiene un monto. ¿Desea sobrescribirlo con el estimado?' })
                                        if (!ok) return
                                      }
                                      // Preparar datos históricos (meses 1..11)
                                      const historicos = rows
                                        .filter(x => x.mes < 12)
                                        .sort((a, b) => a.mes - b.mes)
                                        .map(x => Number(x.monto))
                                      const alpha = 0.7
                                      const pronosticoInicial = historicos[0] || 0
                                      const { pronosticos } = SuavizamientoExponencialService.suavizamientoExponencialSimple(historicos, alpha, pronosticoInicial)
                                      const estimadoMes12 = pronosticos[pronosticos.length - 1]
                                      handleChange(12, Math.round(estimadoMes12))
                                      setToast({ type: 'success', message: 'Mes 12 estimado por Suavizamiento Exponencial.', ts: Date.now() })
                                    } catch (err) {
                                      await alert({ title: 'Error', message: err?.message || 'No se pudo generar mes 12.' })
                                    }
                                  }}
                                  variant="outline"
                                  size="sm"
                                  className="cursor-pointer"
                                  title="Generar estimación para Mes 12"
                                >Generar M12</Button>
                              )}
                            </div>
                          </td>
