...
export const useAnalisisBalance = () => {
   ...
   */
  const exportarCSV = useCallback((datos, tipo, nombreArchivo = 'analisis_balance.csv') => {
    ...
    if (tipo === 'vertical') {
      // Encabezados para análisis vertical
      csv = 'Código,Nombre,Sección,Monto,Denominador,Porcentaje\n'
      
      datos.forEach(linea => {
        const porcentaje = linea.porcentaje !== null 
          ? (linea.porcentaje * 100).toFixed(2) + '%' 
          : 'N/D'
        csv += `"${linea.codigo}","${linea.nombre}","${linea.seccion}",${linea.monto},${linea.denominador},${porcentaje}\n`
      })
    } else if (tipo === 'horizontal') {
      // Encabezados para análisis horizontal
      csv = 'Código,Nombre,Sección,Monto Base,Monto Comparación,Variación Absoluta,Variación %\n'
      
      datos.forEach(linea => {
        const variacionPct = linea.variacion_pct !== null 
          ? (linea.variacion_pct * 100).toFixed(2) + '%' 
          : 'N/D'
        csv += `"${linea.codigo}","${linea.nombre}","${linea.seccion}",${linea.monto_base},${linea.monto_comp},${linea.variacion_abs},${variacionPct}\n`
      })
    }
